...
(async () => {
  ...
  const url = 'https://etherscan.io/token/generic-tokenholder-inventory?m=normal&contractAddress=0x0bd4d37e0907c9f564aaa0a7528837b81b25c605&a=&p='
  ...
  let totalPages = parseInt(pageText.substring(pageText.indexOf('f')+2))
  ...
  for (i = 1; i < totalPages+1; i++) {
    ...
    let looped = 0;
    const totalTokens = $('p:first').text().trim();
    ...
    console.log('Addresses Below will be less than tokens found as duplicates are removed so the address is only found once throught each list')
    console.log('-----------------------------')
    const addresses = [];
    for (i in (tId)) {
    ...
    const combinedPassHolder = './CombinedPassHolders.csv';
    const goldPassHolder = './GoldPassHolders.csv';
    const silverPassHolder = './SilverPassHolders.csv';
    ...
    console.log('Amounts should be 1 less than etherscans amount since llamas address is removed from the list!')
